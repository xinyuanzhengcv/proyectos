...
pd.set_option('display.width', 150)            # No cortar el ancho de la línea
pd.set_option('display.max_colwidth', None)    # Mostrar contenido completo de cada celda
pd.set_option('display.float_format', lambda x: f"{x:.3e}" if abs(x) >= 1e9 else f"{x:.3f}") # Valores cientificos para >10 cifras
# --------------------------------------------------------------------------------------------------------------------------------



##################################################################################################################################
## MODELOS MACHINE LEARNING PYCARET()
##################################################################################################################################
...
# === 1. Cargar datos desde el parquet ===
df = pd.read_parquet("datos.parquet")

# === 2. Setup del entorno de PyCaret ===
reg_setup = setup(
    data=df,
    target='chamartin',
    session_id=123456,
...
# === 4. Finalizar modelo (entrenar en todo el train set) ===
final_model = finalize_model(best_model)
print(final_model)
# === 5. Evaluación visual de resultados ===
evaluate_model(final_model)

# === 6. Guardar el mejor modelo ===

save_model(final_model, os.path.join(base_dir, 'mejor_modelo_chamartin'))
